...
def build_model(input_dim):
  ...
  # linear layer for hazard function
...
def cox_loss(y_true, y_pred):
  ...
  # Sort by predicted hazard, keeping track of indices
  sorting_idx = tf.argsort(y_pred, direction='DESCENDING')
  T_sorted = tf.gather(Sur_Time, sorting_idx)
  E_sorted = tf.gather(Event_Time, sorting_idx)

  # Calculate risk scores using the predicted hazard rates
  risk_scores = K.exp(y_pred)

  # Get the log partial likelihood
  log_likelihood = K.cumsum(E_sorted * tf.math.log(risk_scores))
  total_likelihood = K.sum(log_likelihood) - K.sum(K.sum(risk_scores))

  # The Cox loss is negative log-likelihood
  return -total_likelihood


## 7-Prepare data for training
y_train = np.column_stack((y_reg_train, y_class_train))  # Combine survival times and event indicators
...
def evaluate_model(model, X_test, y_test):
    ...
    # Calculate C-index
    c_index = concordance_index(y_reg_test, -predictions.flatten(), y_class_test)
    print(f"C-Index: {c_index:.4f}")

    return predictions

y_test = np.column_stack((y_reg_test, y_class_test))
